...
describe.skipIf(!TEST_DATABASE_URL)("postgres persistence capabilities", () => {
  test.each([false, true])(
    "round-trips storage, checkpoints, operations, and outputs (legacy=%s)",
    async (legacy) => {
      ...
      try {
        ...
        const now = new Date();
        const storage = await store.insertWorkspaceStorage({
          id: storageId,
        ...
        if (legacy) {
          await sql.unsafe(
            `UPDATE "${schema}".workspace_storage SET provider_ref = $1::jsonb, mount_manifest = $2::jsonb WHERE id = $3`,
            [JSON.stringify(storage.providerRef), JSON.stringify(storage.mountManifest), storageId],
          );
          await sql.unsafe(
            `UPDATE "${schema}".workspace_checkpoints SET provider_ref = $1::jsonb,
           template_snapshot = $2::jsonb, manifest = $3::jsonb WHERE id = $4`,
            [
              JSON.stringify(checkpoint.providerRef),
              JSON.stringify(checkpoint.templateSnapshot),
              JSON.stringify(manifest),
              checkpointId,
            ],
          );
        }
        expect(await store.getStorage(storageId)).toEqual(storage);
        expect(await store.getCheckpoint(checkpointId)).toEqual(checkpoint);
